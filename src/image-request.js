...
class ImageRequest {
    ...
    async setup(event) {
        try {
            if (process.env.SAFE_URL == "True") {
               this.event = this.verifySecurityHash(event);
            }
    ...
    /**
     * verifies that the security hash sent in the path is valid and removes it from the requested path.
     * @param {Object} event - The request body.
     */
    verifySecurityHash(event) {
        ...
        var hash = crypto.createHmac('sha1', securityKey).update(path).digest('base64').replace(/\+/g, "-").replace(/\//g, "_");

        if (securityHash !== hash) {
            throw ({
                status: 403,
                code: 'ThumborMapping::VerifySecurityHash::InvalidSecurityHash',
                message: 'Invalid security key for path.'
            });
        }
